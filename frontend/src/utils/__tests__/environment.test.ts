...
import { describe, expect, it } from "vitest"; // ← Added 'it' import
import {
  config,
  getS3Url,
  isFeatureEnabled,
...
describe("environment config", () => {
  ...
  describe("utility functions", () => {
    it("generates S3 URLs correctly", () => {
      expect(getS3Url("test-path")).toBe(
        "https://mifugo360-biometrics.s3.af-south-1.amazonaws.com/test-path"
      );
    });

    // REMOVED duplicate phone validation test - it's already tested above
  });
});
