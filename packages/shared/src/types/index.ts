...
export interface Livestock {
  ...
  /** Breed of the animal, e.g. "Friesian" or "Galla" (reference data). */
  breed?: string;
  ...
  lng: number;
  createdAt?: string;
  /** Optional reference to the Farmer who owns this animal. */
  farmerId?: number;
 ...
 * Farmer entity.
 *
 * Every farmer has a human-readable name plus an anonymised registration
 * code (e.g. FM-0042) so records can be referenced without exposing PII.
 */
export interface Farmer {
  id: number;
  /** Anonymised code, e.g. "FM-0042". */
  code: string;
  name: string;
  phone: string;
  county: string;
