...
const AnimalDetailSheet = forwardRef<AnimalDetailSheetHandle, AnimalDetailSheetProps>(
  ({ animal, onClose, onSaved }, ref) => {
    ...
    const [editType, setEditType] = useState<AnimalType>(animal?.type ?? 'Cattle');
    const [editBreed, setEditBreed] = useState(animal?.breed ?? '');
    const [editCounty, setEditCounty] = useState(animal?.county ?? '');
    const [editOwner, setEditOwner] = useState(animal?.owner ?? '');
    const [editHealth, setEditHealth] = useState<HealthStatus>(animal?.health ?? 'Healthy');
