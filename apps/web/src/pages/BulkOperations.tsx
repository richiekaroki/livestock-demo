  ...
  const handleBulkAction = async () => {
    ...
    try {
      if (bulkAction === "health") {
        ...
        if (!confirm(t("Delete selected animals?", "Delete selected animals?"))) {
          setExecuting(false);
          return;
        }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[var(--color-text)]">
          {t("Bulk Operations", "Bulk Operations")}
        </h1>
        <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
          {t("Select multiple animals for bulk actions", "Select multiple animals for bulk actions")}
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <select
          ...
          className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm text-[var(--color-text)]"
        >
          <option value="">{t("All Health", "All Health")}</option>
        ...
        <span className="text-sm text-[var(--color-text-secondary)]">
          {selected.size} / {filtered.length} {t("selected", "selected")}
        </span>
      </div>
      ...
      {selected.size > 0 && (
        <div className="bg-[var(--color-primary)]/5 border border-[var(--color-primary)]/20 rounded-xl p-4 mb-4 flex flex-wrap items-center gap-3">
          <span className="text-sm font-medium text-[var(--color-text)]">
            {selected.size} {t("animals selected", "animals selected")}
          </span>
          <select
            value={bulkAction}
          ...
          >
            ...
            <option value="health">{t("Change Health Status", "Change Health Status")}</option>
            <option value="export">{t("Export Selected", "Export Selected")}</option>
            <option value="delete">{t("Delete Selected", "Delete Selected")}</option>
          </select>
          {bulkAction === "health" && (
            <select
              value={bulkHealth}
          ...
          >
            {executing ? t("Processing...", "Processing...") : t("Execute", "Execute")}
          </button>
        </div>
      )}
        ...
        <table className="w-full text-sm">
          <thead className="bg-[var(--color-bg)] border-b border-[var(--color-border)]">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-medium text-[var(--color-text)]">{t("Name", "Name")}</th>
              <th className="px-4 py-3 text-left font-medium text-[var(--color-text)]">{t("Type", "Type")}</th>
              <th className="px-4 py-3 text-left font-medium text-[var(--color-text)]">{t("Health", "Health")}</th>
              <th className="px-4 py-3 text-left font-medium text-[var(--color-text)]">{t("County", "County")}</th>
              <th className="px-4 py-3 text-left font-medium text-[var(--color-text)]">{t("Owner", "Owner")}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--color-border)]">
            {loading ? (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-[var(--color-text-secondary)]">{t("Loading...", "Loading...")}</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-[var(--color-text-secondary)]">{t("No animals found", "No animals found")}</td></tr>
            ) : (
              filtered.map((a) => (
                <tr key={a.id} className={`hover:bg-[var(--color-bg)] ${selected.has(a.id) ? "bg-[var(--color-primary)]/5" : ""}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                      ...
                      a.health === "Sick" ? "bg-red-100 text-red-800" :
