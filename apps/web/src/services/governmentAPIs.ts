 ...
 *
 * Sandbox stubs — KALRO/KIAMIS have no public sandbox credentials; replace with
 * live integrations when credentials exist. The service boundary stays identical.
 */
...
class GovernmentAPIService {
  ...
  async reportDiseaseOutbreak(data: {
    ...
    try {
      if (this._useRemoteBackend) {
        const res = await remoteApi.reportDiseaseOutbreak({
